...
import {fetchAllOrderByActionDate, fetchAllOrderByCleaningDate, fetchOrder, updateOrder} from "../API/order";
...
function* updateOrderAsync(order) {
    try {
        ...
        yield put({type: "app/changeFormAction", payload: {action: "RESET_FORM"}});
        ...
        yield put({type: "notification/SHOW_NOTICE", payload: {type: "error", "message": 'Ошибка при сохранении!'}});
    } finally {
...
function* updateEvents(action) {
    ...
    try {
        ...
        const [cleanings, actions, schedule, foremans] = yield all([
            call(fetchAllOrderByCleaningDate, fetchDate(date).startOf('month').subtract(2, 'week').toISOString(), fetchDate(date).endOf('month').add(2, 'week').toISOString()),
            call(fetchAllOrderByActionDate, fetchDate(date).startOf('month').subtract(2, 'week').toISOString(), fetchDate(date).endOf('month').add(2, 'week').toISOString()),
            call(fetchSchedule, fetchDate(date).startOf('month').subtract(2, 'week').toISOString(), fetchDate(date).endOf('month').add(2, 'week').toISOString()),
        ...
        const emptySlots = schedule?.flatMap(daySchedule => {
            const available = daySchedule.personnel.filter(personnel => personnel.status === "WORKING" && foremans.some(f=>f.id===personnel.personnelId)).length;
            const cleaningsAtDay = cleanings.filter(cleaning => cleaning.serviceType !== "Поддерживающая уборка" &&
                fetchDate(cleaning.cleaningDate.startDate).startOf('day').isSameOrBefore(daySchedule.date) &&
                fetchDate(cleaning.cleaningDate.endDate).endOf('day').isSameOrAfter(daySchedule.date));
        ...
        const eventsForCalendar = actions.reduce((accum, _order) => {
            ...
            if (order.stageName) {
                ...
                if (order.stageName === 'ORDER_CREATION') {//Если Заявка только создана, видит ее только создавший
                ...
                } else if ((order.stageName === 'RESERVE_ORDER' || order.stageName === 'CHANGE_COST')) {//??
                    (order.responsibleUsers.includes(user.id) || user.role === "OPERATOR") &&
                    events.push({
                        ...order,
                        events: [],
                        stageGroup: StageManager.defineStageGroup(order.stageName),
                ...
                } else if (StageManager.checkAvailableStageByRole(order.stageName, user.role) && !StageManager.hideEvents(order.stageName, user.role)) {
        ...
        yield put({//Нужно обсудить как, кто и что должны видеть, либо все, либо конкретный человек
...
function* orderToArchive(action) {
    yield put({type: "app/changeFormAction", payload: {action: "TO_ARCHIVE", reason: action.payload}});
    // yield put({type: "app/changeFormAction", payload: {action:"NONE"}});
